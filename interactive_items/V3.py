...
def run(screen, inventory=None):
	...
	# Projection state: only show after hourglass is picked up and C is pressed
	group_img = None
	group_img_alpha = 0  # For fade-in/out
	group_img_fadein = False
	group_img_fadeout = False
	group_img_fully_visible = False
	show_projection = False
	projection_text_lines = [
		"Against the sand of time,",
		"we measure the weight of memory"
	]
	projection_text_alpha = 0
	projection_text_fadein = False
	projection_text_fadeout = False
	projection_text_fully_visible = False
	projection_timer = 0.0
	projection_fadeout_started = False
	...
	while running:
		...
		dt = min(dt, 0.2)  # Clamp dt to avoid large jumps
		# Handle projection timer and fadeout
		if show_projection and group_img_fully_visible and projection_text_fully_visible and not projection_fadeout_started:
			projection_timer += dt
			if projection_timer >= 5.0:
				group_img_fadeout = True
				projection_text_fadeout = True
				projection_fadeout_started = True
						...
						if dist < max(int(tile_w * 1.5), 48):
							...
							if picked or not inventory:
								if it.get('id') == 'hourglass':
									# Start projection fade-in
									show_projection = True
									group_img_fadein = True
									projection_text_fadein = True
									if group_img is None:
										try:
											group_img_path = os.path.join('assets', 'group.png')
											group_img = pygame.image.load(group_img_path).convert_alpha()
			...
			for d in door_rects:
				if player.rect.colliderect(d):
					...
					try:
						...
						# ignore and continue with norm and presses Tal door behaviour
						pass
		...
		# Always show white translucent screen in the middle after pressing C
		...
		if show_projection:
			...
			# Fade in group.png
			if group_img_fadein and not group_img_fully_visible:
				group_img_alpha += int(255 * dt / 1.2)  # 1.2s fade-in
				if group_img_alpha >= 255:
					group_img_alpha = 255
					group_img_fadein = False
					group_img_fully_visible = True

			# Fade out group.png
			if group_img_fadeout and group_img_alpha > 0:
				group_img_alpha -= int(255 * dt / 1.2)  # 1.2s fade-out
				if group_img_alpha <= 0:
					group_img_alpha = 0
					group_img_fadeout = False

			# Draw group.png onto overlay BEFORE blitting overlay to screen
			if group_img and group_img_alpha > 0:
				img = group_img.copy()
				img.set_alpha(group_img_alpha)
				img_w, img_h = img.get_size()
				img_x = (rect_w - img_w) // 2
				img_y = (rect_h - img_h) // 2
				overlay.blit(img, (img_x, img_y))

			# Now blit overlay (with group.png) to the screen
			screen.blit(overlay, (rect_x, rect_y))

			# Fade in text
			if projection_text_fadein and not projection_text_fully_visible:
				projection_text_alpha += int(255 * dt / 1.2)
				if projection_text_alpha >= 255:
					projection_text_alpha = 255
					projection_text_fadein = False
					projection_text_fully_visible = True

			# Fade out text
			if projection_text_fadeout and projection_text_alpha > 0:
				projection_text_alpha -= int(255 * dt / 1.2)
				if projection_text_alpha <= 0:
					projection_text_alpha = 0
					projection_text_fadeout = False

			# Draw text under the white transparent screen, two rows, black color, no background
			if projection_text_lines and projection_text_alpha > 0:
				text_font = pygame.font.Font(os.path.join(ROOT, 'assets', 'Silver.ttf'), 22) if os.path.exists(os.path.join(ROOT, 'assets', 'Silver.ttf')) else pygame.font.SysFont('consolas', 22)
				total_height = 0
				text_surfs = []
				for line in projection_text_lines:
					surf = text_font.render(line, True, (0, 0, 0))
					surf.set_alpha(projection_text_alpha)
			...
			# Show 1-1-1.png in the center of the white translucent screen after group.png and text disappear
			# Add click prompt and move to bottom left after click
			if show_projection and not group_img_fadeout and not projection_text_fadeout and projection_fadeout_started:
				if group_img_alpha == 0 and projection_text_alpha == 0:
					try:
						reward_img_path = os.path.join('assets', '1-1-1.png')
						if os.path.exists(reward_img_path):
							...
							# Track click state
							if 'reward_img_clicked' not in globals():
								global reward_img_clicked
								reward_img_clicked = False
							...
							if not reward_img_clicked:
								...
								# Draw 'click' prompt at upper right of the image
								...
								# Move the reward image to the bottom left corner of the screen
								margin = 16
								img_x = margin
		...
		if 'show_brush_projection' in locals() and show_brush_projection:
			if 'brush_proj_timer' not in locals():
				brush_proj_timer = 0.0
			brush_proj_timer += dt
			# White translucent overlay (already drawn above)
			# Fade in brush.png
			try:
				if 'brush_img' not in locals():
					brush_img_path = os.path.join('assets', 'brush.png')
					if os.path.exists(brush_img_path):
						brush_img = pygame.image.load(brush_img_path).convert_alpha()
				...
				if brush_img and brush_proj_timer <= 5.0:
					...
					img_x = rect_x + (rect_w - img.get_width()) // 2
					img_y = rect_y + 20
			...
			if brush_proj_timer > 5.0:
				...
				show_121_img = True
				if 'img_121_clicked' not in globals():
					global img_121_clicked
					img_121_clicked = False

		# --- Show 1-2-1.png after brush projection ---
		if 'show_121_img' in locals() and show_121_img:
			try:
				img_121_path = os.path.join('assets', '1-2-1.png')
						...
						if ev.button == 1:
							if not img_121_clicked:
								...
								if img_rect.collidepoint(mx, my):
									...
									# Start timer for showing the text
									global show_assemble_text, assemble_text_timer
									show_assemble_text = True
									assemble_text_timer = 0.0
					if not img_121_clicked:
						img_x = rect_x + (rect_w - 80) // 2
						img_y = rect_y + (rect_h - 80) // 2
		...
		# --- Show assemble gears text after clicking 1-2-1.png ---
		if 'show_assemble_text' in globals() and show_assemble_text:
			if 'assemble_text_timer' not in globals():
				assemble_text_timer = 0.0
			assemble_text_timer += dt
			# Draw the text in the middle of the white screen
			text = "Try to assemble the gears here..."
			text_font = pygame.font.Font(os.path.join(ROOT, 'assets', 'Silver.ttf'), 22) if os.path.exists(os.path.join(ROOT, 'assets', 'Silver.ttf')) else pygame.font.SysFont('consolas', 22)
			text_surf = text_font.render(text, True, (0, 0, 0))
			text_x = rect_x + (rect_w - text_surf.get_width()) // 2
			text_y = rect_y + (rect_h - text_surf.get_height()) // 2
			screen.blit(text_surf, (text_x, text_y))
			if assemble_text_timer > 4.0:
				show_assemble_text = False
				global show_gear_placeholders
				show_gear_placeholders = True


		# --- Show two gear-shaped semi-transparent gray areas after text disappears ---
		if 'show_gear_placeholders' in globals() and show_gear_placeholders:
			# Draw two gear placeholders in the center of the white screen
			gear_size = 80
			gear_alpha = 120
			gear_color = (120, 120, 120, gear_alpha)
			# Calculate positions: left and right of center
			gear1_x = rect_x + rect_w//2 - gear_size - 10
			gear2_x = rect_x + rect_w//2 + 10
			gear_y = rect_y + (rect_h - gear_size)//2
			def draw_gear_placeholder(surf, x, y, size, color):
				# Draw a simple gear shape: circle with notches
				import math
				temp = pygame.Surface((size, size), pygame.SRCALPHA)
				pygame.draw.circle(temp, color, (size//2, size//2), size//2-4)
				# Draw notches
				for i in range(8):
					angle = i * (2*math.pi/8)
					notch_x = int(size//2 + (size//2-6) * math.cos(angle))
					notch_y = int(size//2 + (size//2-6) * math.sin(angle))
					pygame.draw.circle(temp, color, (notch_x, notch_y), 7)
				surf.blit(temp, (x, y))
			draw_gear_placeholder(screen, gear1_x, gear_y, gear_size, gear_color)
			draw_gear_placeholder(screen, gear2_x, gear_y, gear_size, gear_color)


		# draw items and prompt if player is near hourglass or lamp, but only one at a time
		for it in items:
			try:
		...
		for it in items:
			show_prompt = False
			if it.get('id') == 'hourglass':
				if 'reward_img_clicked' in globals() and reward_img_clicked:
					show_prompt = False
				else:
					show_prompt = True
			elif it.get('id') == 'lamp':
