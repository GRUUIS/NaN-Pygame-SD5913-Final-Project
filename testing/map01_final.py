...
def run(screen):
	...
	while running:
		...
		for ev in pygame.event.get():
			# Drag logic for reward images
			if show_121_img and img_121_clicked:
			...
			if group_img_fadeout and group_img_alpha == 0 and reward_img_clicked:
				if ev.type == pygame.MOUSEBUTTONDOWN and ev.button == 1:
					if reward_img_pos:
				...
				if ev.key == pygame.K_ESCAPE:
					...
					for it in list(items):
						...
						if dist < max(tile_w * 6, 160):
							item_id = it.get('id')
							if item_id == 'lamp':
								# Remove lamp and trigger brush projection
								items.remove(it)
								...
								# Remove hourglass and trigger group.png
								items.remove(it)
								collected_items.add(item_id)
								print('Picked up', item_id)
								image_fadein = True
			...
			try:
				...
				if os.path.exists(img_121_path):
					...
					if not img_121_clicked:
						...
						# Draw two gears in the center
						gear_center1 = (screen.get_width()//2 - 80, screen.get_height()//2)
						gear_center2 = (screen.get_width()//2 + 80, screen.get_height()//2)
						draw_gear(screen, gear_center1, 90, teeth=12, color=(120,120,120,128))
						draw_gear(screen, gear_center2, 90, teeth=12, color=(120,120,120,128))
