...
def run(screen):
	...
	try:
		if platforms:
			...
			# Prefer a packaged UI hourglass if present, otherwise fall back to other locations
			explicit_ui = os.path.join('assets', 'UI', 'hourglass.png')
			if os.path.exists(explicit_ui):
				img_path = explicit_ui
			else:
				img_path = _find_asset('hourglass.png') or _find_asset('item_clock.png') or _find_asset('clock.png')
			if img_path:
				try:
