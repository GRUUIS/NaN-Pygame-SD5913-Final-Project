...
def run(screen):
		...
		if platforms:
			...
			try:
				...
				if lamp_img_path:
					...
					# Try placing lamp 120 pixels to the right
					lamp_x = item_x + item_w + 120
					
					# Check if it fits on the platform (optional, but good for visuals)
					# If it goes off the platform to the right, try placing to the left
					if lamp_x + lamp_w > top_plat.right:
						lamp_x = item_x - 120 - lamp_w
	...
	while running:
		...
		# Move overlay to bottom to avoid obscuring player/items at the top
		rect_y = screen_h - rect_h - 60
